const { errorCodes, AppError, assert, RequestRule, AbstractLogger } = require('../lib')
...
class BaseController {
  ...
  actionRunner (action) {
    ...
    return async (req, res, next) => {
      ...
      try {
         ...
         */
        if (action.validationRules) { // create a validation for mongodb
  ...
  validateSchema (src, requestSchema, schemaTitle) {
    ...
    const defaultValidKeys = ['offset', 'page', 'limit', 'filter', 'orderBy', 'fields']
